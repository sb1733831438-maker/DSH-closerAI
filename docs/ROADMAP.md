...
| **v0.9.x**    | 签名/分发 + 渲染层测试收尾 + 权限审批 UX                    | 代码签名、B-1 权限审批、发布自动化 | 签名未落地即阻塞      |
...
> **状态 2026-08-27（v0.8.0）**：A-1、A-2、A-3（除 R-14 签名）、A-4、A-5 已在
> `release/v0.8.0-hardening` 分支全部落地并验证（详见 `docs/STATUS.md`）。
> 剩余：**R-14 代码签名**（等待购买证书，阻塞项）、B 系列 1.0 项。

> 每一项都映射到 REVIEW 的 R- 编号与 BENCHMARK 的 L- 编号，并给出验收标准。

### A-1. 修复 P1 缺陷（v0.8.x 核心）— ✅ 已全部落地
...
### A-2. 修复已确认的渲染层与契约缺陷（v0.8.x）— ✅ 已全部落地
...
### A-3. 安全加固（v0.8.x~v0.9.x）— ✅ 除 R-14 签名外已落地
...
### A-4. MCP 运行时挂载（v0.8.0 提前完成）— ✅ 已落地并 E2E 验证
