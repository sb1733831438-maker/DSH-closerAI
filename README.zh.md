...
**预发布 —— v0.0.7。** 项目正在向 v0.1.0 可日常使用版本持续开发。当前里程碑见
...
| v0.0.5 | 日常对话：会话持久化、历史、项目、文件处理                        | 已发布 |
| v0.0.6 | 扩展与 Web：搜索、MCP、技能、插件清单、Agent UI                   | 进行中 |
...
- 会话持久化：会话保存在稳定的本地目录中，重启后可恢复；「CloserAI 工作区」管理页（菜单 /
  `Ctrl+Shift+M`）可列出项目与会话历史，并支持切换、删除、导出与导入（原生目录选择器）。
- 项目：命名的 Chat/Work/Code 项目，可指定代码工作目录；重启后自动恢复当前项目。
